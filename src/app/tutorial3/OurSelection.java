...
public class OurSelection extends SelectionMethod {
	...
	public static final String P_MIDDLEPROBABILITY = "middle-probability"; // our
																			// parameter
																			// name
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		// database, returning a value of min-1 (-1.0) if the parameter doesn't
		// exist or was
		// outside this range.
	...
	public int produce(final int subpopulation, final EvolutionState state, final int thread) {
		...
		if (state.random[thread].nextBoolean(middleProbability)) {
			...
			// so you shouldn't see any cycles here except in very unusual
			// domains...
			if (inds[two].fitness.betterThan(inds[one].fitness)) {
				if (inds[three].fitness.betterThan(inds[two].fitness)) // 1 < 2
																		// < 3
					return two;
				else if (inds[three].fitness.betterThan(inds[one].fitness)) // 1
																			// <
																			// 3
																			// <
																			// 2
					...
					return one;
			} else if (inds[three].fitness.betterThan(inds[one].fitness)) // 2 <
																			// 1
																			// <
																			// 3
				return one;
			else if (inds[three].fitness.betterThan(inds[two].fitness)) // 2 < 3
																		// < 1
				return three;
			else // 3 < 2 < 1
				return two;
